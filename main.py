...
UPLOAD_FOLDER = current_file_directory + str('/uploads')
PROCESSED_FOLDER = 'processed'
ALLOWED_EXTENSIONS = {'processed', 'docx', 'pdf'}
...
#        file_path = os.path.join(app.config['UPLOAD_FOLDER'], filename)
        file_path = f"{current_file_directory}/uploads/{filename}"
        file.save(file_path)
        ...
        if model == "Gemini":
            print(template)
            gemini.gemini_process(Input, filename,template)
        else:
            chatgpt.chatgpt_process(Input, filename,template)

        output_filename = filename
#        output_path = os.path.join(app.config['PROCESSED_FOLDER'], output_filename)
        output_path=f"{current_file_directory}/processed/{output_filename}"
        empty_folder(UPLOAD_FOLDER)
        return jsonify({'filename': output_filename, 'url': url_for('uploaded_file', filename=output_filename.split('.')[0]+".docx")})
    else:
